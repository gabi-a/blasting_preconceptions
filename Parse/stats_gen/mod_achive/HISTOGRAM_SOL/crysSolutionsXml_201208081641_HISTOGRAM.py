...
# $Id: crysSolutionsXml_201208081641_HISTOGRAM.py 125 2012-12-11 03:42:55Z cha68f $
...
#HISTO_COUNT=50                                             #Number of coordinates wanted for per histogram
NOT_AVAILABLE=-999
NOT_SET = -2
TRUE = 0
FALSE = 1
...
def main():
	...
	cur.execute("SELECT Chemicals.Name, Chemicals.Chemical_id, Crystal_SOLUTIONs.CONCENTRATION, Crystal_SOLUTIONs.PH, Crystal_SOLUTIONs.UNITS FROM Chemicals INNER JOIN Crystal_SOLUTIONs ON Chemicals.Chemical_id=Crystal_SOLUTIONs.Chemical_id ORDER BY Chemicals.Name")
	...
	curr_unit = db_result[0][4]
	multi_unit_found = FALSE
	
	for chem in db_result:
		if chem[0] == curr_chem:
			if (chem[4] != curr_unit) and (multi_unit_found == FALSE):
	...
	while row < len(db_result):
		...
		if curr_name == prev_name:
			...
			if (curr_name in multi_unit_tracker) and (invalid_unit == FALSE):
				if mod_unitConverter.get_unit_type(db_result[row][4],density) != NOT_AVAILABLE:
					curr_conc = mod_unitConverter.convert_unit_to_wv(db_result[row][4],db_result[row][2],density,mw)
			...
			if set_wv_unit == TRUE:
				...
				units = db_result[row-1][4]
			
			if (invalid_unit == FALSE) and (prev_name.lower() != "unknown"):
				maxx,maxy,coords = crysSolutionsXml_func.histo_coords(prev_name,conc)
